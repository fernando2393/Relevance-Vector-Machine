...
data_set = "banana"
datas = 10
total_support_vector = np.zeros(datas)
predict_error = np.zeros(datas)
for index in range(datas):
    ...
    total_support_vector[index] = sum(svclassifier.n_support_)
    print(total_support_vector[index])
        #### ERROR
    predict_error[index] = get_prediction_error_rate(y_pred, test_labels)
    ...
    # plt.show()
print("mean of number of support vectors: {tsv}".format(tsv = np.mean(total_support_vector)))
print("mean of the predict error is {error}".format(error = np.mean(predict_error)))
