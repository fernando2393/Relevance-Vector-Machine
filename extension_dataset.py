...
for i in range(X.shape[0]):
    y_train[i] = math.sin(X[i, 0]) / X[i, 0] + 0.1 * X[i, 1] + np.random.normal(0,0.1)
...
alpha, variance_mp, mu_mp, sigma_mp = rvm_r.fit(X, variance, y_train, kernel, X.shape[0])
...
y_pred = rvm_r.predict(X, X_test, relevant_vectors, variance_mp, mu_mp, sigma_mp, kernel, dimensions)
...
ax.view_init(30, -50)
ax.scatter(X[:,0], X[:,1], y_train, c = "black", label='Training points')
